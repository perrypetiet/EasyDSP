  ...
  if(toSettings != NULL)
  {
    switch(ctxt->op)
    {
      ...
      case BLE_GATT_ACCESS_OP_WRITE_CHR:
        ...
        currentEq.q = (float)(*(ctxt->om->om_data) / (float)10.0);
        if(!send_current_eq())
        {
          currentEq.q = old;
        }
  ...
  if(toSettings != NULL)
  {
    switch(ctxt->op)
    {
      ...
      case BLE_GATT_ACCESS_OP_WRITE_CHR:
        ...
        currentEq.s = (float)(*(ctxt->om->om_data) / (float)10.0);
        if(!send_current_eq())
        {
          currentEq.s = old;
        }
  ...
  if(toSettings != NULL)
  {
    switch(ctxt->op)
    {
      ...
      case BLE_GATT_ACCESS_OP_WRITE_CHR:
        ...
        currentEq.boost = (float)(*(ctxt->om->om_data) / (float)10.0);
        if(!send_current_eq())
        {
          currentEq.boost = old;
        }
  ...
  if(toSettings != NULL)
  {
    switch(ctxt->op)
    {
      ...
      case BLE_GATT_ACCESS_OP_WRITE_CHR:
        ...
        currentEq.gain = (float)(*(ctxt->om->om_data) / (float)10.0);
        if(!send_current_eq())
        {
          currentEq.gain = old;
        }
...
bool init_ble(uint8_t* name, communication_t* communication_data)
{
  ...
  /* TODO: This uses a lot of flash :( Can maybe be done different?  */
